...
if [[ "${MODE}" == "print-env" ]]; then
  ...
  if [[ -n "${SESSION_DIR}" && -f "${SESSION_DIR}/cpuapp.env" ]]; then
    cat "${SESSION_DIR}/cpuapp.env"
    exit 0
  fi
  echo "cpuapp.env not found. Start a session first with: cpu.app --start-session" 1>&2
  exit 1
fi
...
if [[ "${MODE}" == "start-session" ]]; then
  ...
  echo "Usage: cpu.app [--geometry WIDTHxHEIGHT] --start-session | --print-env | cpu.app <application> [args...]" 1>&2
  exit 1
fi
...
# CPU sessions should render fully in software to avoid libEGL /dev/dri permission warnings.
...
create_env_file () {
  (
    umask 077
    cat > "${SESSION_DIR}/cpuapp.env" <<EOF
...
export GALLIUM_DRIVER="llvmpipe"
EOF
    cp -f "${SESSION_DIR}/cpuapp.env" "/home/$USER/cpuapp.env"
  )
}
...
change_passwd() {
  echo -ne "$password
$spassword" | singularity exec -B ${HOME}:${HOME} -B /usr/lib/locale:/usr/lib/locale,/var:/var,/tmp:/tmp /apps/software/extern/singularity/app.GUI/2.0/rockylinux9.sif /opt/TurboVNC/bin/vncpasswd -f > "${SESSION_DIR}/vnc.passwd" 2>/dev/null || true
...
echo "Shell environment saved to ${SESSION_DIR}/cpuapp.env"
echo "Run: source ${HOME}/cpuapp.env"
...
(
  ...
  fluxbox () {
    SINGULARITYENV_XAUTHORITY=${XAUTH_FILE}     SINGULARITYENV_DISPLAY=":${display}"     SINGULARITYENV_QNVSM="${QNVSM}"     SINGULARITYENV_XDG_RUNTIME_DIR="/run/user/$(id -u)"     singularity exec -B /usr/lib/locale/:/usr/lib/locale/,/var:/var,/tmp:/tmp -B ${XAUTH_FILE}:${XAUTH_FILE} -B "${RUN_USER_DIR}":/run/user/$(id -u) /apps/software/extern/singularity/app.GUI/2.0/rockylinux9.sif fluxbox "$@"
  }
...
if [[ ${SESSION_ONLY} -eq 1 ]]; then
  ...
  echo 'Run: eval "$(cpu.app --print-env)"'
