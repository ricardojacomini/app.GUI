...
export GALLIUM_DRIVER="llvmpipe"
EOF
    cp -f "${SESSION_DIR}/cpuapp.env" "${HOME}/.vnc/cpuapp.env"
  )
}
...
change_passwd() {
  echo -ne "$password
$spassword" | singularity exec -B ${HOME}:${HOME} -B /usr/lib/locale:/usr/lib/locale,/var:/var,/tmp:/tmp ${SIF_IMAGE} /opt/TurboVNC/bin/vncpasswd -f > "${SESSION_DIR}/vnc.passwd" 2>/dev/null || true
...
echo "Run: source ${HOME}/.vnc/cpuapp.env"
...
(
  ...
  fluxbox () {
    SINGULARITYENV_XAUTHORITY=${XAUTH_FILE}     SINGULARITYENV_DISPLAY=":${display}"     SINGULARITYENV_QNVSM="${QNVSM}"     SINGULARITYENV_XDG_RUNTIME_DIR="/run/user/${MY_UID}"     singularity exec -B /usr/lib/locale/:/usr/lib/locale/,/var:/var,/tmp:/tmp -B ${XAUTH_FILE}:${XAUTH_FILE} -B "${RUN_USER_DIR}":/run/user/${MY_UID} ${SIF_IMAGE} fluxbox "$@"
  }
