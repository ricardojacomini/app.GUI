...
(
  ...
  fluxbox () {
    SINGULARITYENV_XAUTHORITY=${XAUTH_FILE}     SINGULARITYENV_DISPLAY=":${display}"     SINGULARITYENV_QNVSM="${QNVSM}"     SINGULARITYENV_XDG_RUNTIME_DIR="/run/user/$(id -u)"     singularity exec -B /usr/lib/locale/:/usr/lib/locale/,/var:/var,/tmp:/tmp -B ${XAUTH_FILE}:${XAUTH_FILE} -B "${RUN_USER_DIR}":/run/user/$(id -u) /data/apps/extern/singularity/app.GUI/2.0/rockylinux9.sif fluxbox "$@"
  }
